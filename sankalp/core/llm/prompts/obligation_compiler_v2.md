...
# Quantities are FLOORS, not exact counts

**Always use `gte` for a quantity, never `eq`.**

This system separates two concerns that look similar but are checked by
different mechanisms:

- **Quantity criteria set a minimum.** "Order 2 Margherita" compiles to
  `quantity_sum gte 2` — *at least* 2. Not `eq 2`.
- **Over-ordering is caught by the budget ceiling**, not by the quantity check.
  If the agent adds a third pizza, the cost check catches it. The quantity check
  exists to catch the agent delivering *too little*, which is the failure the
  user cannot see on a receipt.

Using `eq` here is a serious error: it blocks legitimate orders. A cart that
contains a free extra item, a substituted equivalent, or a rounded-up pack size
still satisfies what the user asked for, and blocking it means a correct order
gets stopped and a real person is inconvenienced for nothing.
...
Instruction: `Order 2 Margherita from Pizza Point. No pork. Keep it under Rs 900.`

Note `gte`, not `eq` — an explicit count is still a floor.

```
{
  "criteria": [
    {"field": "quantity_sum", "operator": "gte", "value_span": "2 Margherita",
     "source": "stated", "evidence_span": "Order 2 Margherita"},
    {"field": "item.ingredients", "operator": "excludes", "value_span": "pork",
     "source": "stated", "evidence_span": "No pork"}
  ],
  "budget_ceiling_span": "under Rs 900",
  "delivery_deadline_span": null,
  "merchant_span": "Pizza Point",
  "merchant_category_span": null,
  "prohibited_spans": ["pork"],
  "ambiguity_flags": []
}
```
